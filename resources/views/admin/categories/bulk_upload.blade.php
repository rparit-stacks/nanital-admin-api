@extends('layouts.admin.app', ['page' => $menuAdmin['categories']['active'] ?? "", 'sub_page' => $menuAdmin['categories']['route']['bulk_upload']['sub_active']])

@section('title', __('labels.bulk_upload_categories'))
...
window.BULK_UPLOAD_CFG = {
  uploadUrl: "{{ route('admin.categories.bulk-upload') }}",
  csrf: "{{ csrf_token() }}",
  i18n: {
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <div>
          <h3 class="card-title">{{ __('labels.bulk_upload_categories') }}</h3>
          ...
          <div class="col-12">
            <div class="alert alert-info" role="alert">
              ...
              <ul class="mb-2">
                ...
                <li>{{ __('labels.optional_fields') ?? 'Optional fields' }}: parent_id, parent_title, description, status, requires_approval, commission, background_type, background_color, font_color</li>
              </ul>
              <a href="{{ route('admin.categories.bulk-template') }}" class="btn btn-outline-primary btn-sm">{{ __('labels.download_template') }}</a>
            </div>
          </div>
          <div class="col-12">
            <form id="bulk-upload-form" enctype="multipart/form-data" Method="POST" action="{{ route('admin.categories.bulk-upload') }}">
              @csrf
              ...
              <div class="mt-3 d-flex gap-2">
                ...
                <a href="{{ route('admin.categories.bulk-template') }}" class="btn btn-outline-secondary">{{ __('labels.download_template') }}</a>
              </div>
            </form>
          </div>
        </div>
