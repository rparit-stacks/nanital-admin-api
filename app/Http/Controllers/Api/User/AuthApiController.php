...
class AuthApiController extends Controller
{
    ...
    private function getFirebaseAuth(): array
    {
        try {
            ...
            $serviceAccountPath = storage_path('app/private/settings/service-account-file.json');

            if (! is_file($serviceAccountPath) || filesize($serviceAccountPath) === 0) {
                return [
                    'success' => false,
                    'message' => 'labels.firebase_service_account_missing_or_invalid',
                    'data' => [
                        'error' => 'Service account file missing or empty at storage/app/private/settings/service-account-file.json',
                    ],
                ];
            }
            ...
            if (! is_array($json) || json_last_error() !== JSON_ERROR_NONE) {
                return [
                    'success' => false,
                    'message' => 'labels.firebase_service_account_missing_or_invalid',
                    'data' => ['error' => 'Service account file is not valid JSON'],
                ];
            }
            ...
            foreach (['type', 'project_id', 'client_email', 'private_key'] as $key) {
                if (empty($json[$key]) || ! is_string($json[$key])) {
                    return [
                        'success' => false,
                        'message' => 'labels.firebase_service_account_missing_or_invalid',
                        'data' => ['error' => "Service account JSON missing or invalid field: {$key}"],
                    ];
                }
            }
